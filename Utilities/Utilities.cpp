 ...
 *
 * This function uses the binary exponentiation algorithm, which is efficient and operates in \O(log(power)).
 *
 ...
 * @return The result of (base^power). If the base is negative and odd, the result is negative.
...
cpp_int Utilities::binaryExponentiation(const cpp_int& base, const cpp_int& power) {
    ...
    if (power == 0) return 1;              // x^0 = 1 for any x != 0
