 ...
 * @param x Output parameter representing the coefficient for base in the
 * equation ax + by = gcd(a, b).
 * @param y Output parameter representing the coefficient for modulus in the
 * equation ax + by = gcd(a, b).
 * @return The greatest common divisor (gcd) of base and modulus.
 */
cpp_int ModularArithmetic::extendedGCD(cpp_int base, const cpp_int &modulus,
                                       cpp_int &x, cpp_int &y) {
  if (modulus == 0) {
    x = 1;
    y = 0;
...
/**
 * Computes the modular multiplicative inverse of a given base under a given
 * modulus.
 *
 ...
 * @throws std::runtime_error If the modulus is 1 or if the inverse does not
 * exist (gcd(base, modulus) != 1).
 */
cpp_int
ModularArithmetic::modularMultiplicativeInverse(const cpp_int &base,
 ...
 * @throws std::runtime_error If power and base are both 0 (undefined) or if mod
 * is invalid.
 */
cpp_int ModularArithmetic::modularExponentiation(const cpp_int &base,
                                                 const cpp_int &power,
                                                 const cpp_int &mod) {
  // Edge cases
  if (mod == 1)
