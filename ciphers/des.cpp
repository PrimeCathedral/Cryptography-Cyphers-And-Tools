...
namespace DataEncryptionStandard {
  ...
  const vector<int> kShiftSchedule{1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
  ...
  vector<bitset<size_of_segment>> splitBitset(const bitset<size_of_original> &original) {
    ...
    constexpr unsigned long long mask{bitset<size_of_segment>().flip().to_ullong()};
  ...
  bitset<Size> &rotateBits(bitset<Size> &original, const int bits_to_rotate) {
    ...
    //TODO: Find a way to remove this static_cast
